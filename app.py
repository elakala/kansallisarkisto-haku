...
def nayta_kpi_kortit(osumia: int, data: list, indeksi_avain: str):
    ...
    vuodet = [h["_source"].get(vuosi_kentta) for h in data if h["_source"].get(vuosi_kentta)]

    col1, col2, col3, col4 = st.columns(4)
    col1.metric("🔍 Osumia yhteensä", f"{osumia:,}".replace(",", " "))
    col2.metric("📄 Näytetään", len(data))
    col3.metric("📅 Vanhin asiakirja", min(vuodet) if vuodet else "–")
    col4.metric("📅 Uusin asiakirja", max(vuodet) if vuodet else "–")
